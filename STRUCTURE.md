...
Circuit Shift is isolated in `client/src/game/circuitShift/`. Its React route owns only HUD state, sound opt-in, difficulty/mode selection, and the restart action. The framework-independent `CircuitShiftWorld` owns the 4×4 tile orientation array, selection index, path evaluation, procedural mesh highlighting, pointer/keyboard listeners, deterministic demo rotations, score, local best score, and cleanup. Its browser-local `calm`, `standard`, and `dense` profiles alter only deterministic scramble intensity and score multiplier. `dailyChallenge.ts` independently derives sixteen daily offsets from the device-local date; `dailyStreak.ts` persists only consecutive completed local-date IDs/current-longest counts. The world updates the streak at the real daily-solve boundary and excludes its deterministic demo mode, without fetching or transmitting any data. The canvas adapter retains the same one-engine, one-scene, resize-safe lifecycle used by the first two games. `client/src/game/registerStandardShaders.ts` explicitly registers Babylon’s default material shader pair before each canvas starts, avoiding blank canvases from deferred shader-module loading.
