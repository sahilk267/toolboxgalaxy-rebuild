...
assert(Object.keys(bySlug).join(",") === "mini-sudoku,tango,queens,patches,zip,wend", "The personal-best overview must expose exactly the six genuine browser-local puzzle banks in a stable order.");
assert(Object.values(bySlug).every((record) => record.availableEditions === 7), "Every personal-best card must disclose the actual seven authored editions available in its bank.");
...
console.log("Logic personal-best model verified: six real bank cards, edition deduplication, reached-date streaks, and legacy/unknown/malformed/future exclusion.");
