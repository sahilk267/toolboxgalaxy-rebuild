...
try {
  ...
  check("Privacy evidence ledger", await evaluate("return document.querySelectorAll('.policy-ledger-section').length === 3 && document.querySelector('.policy-readouts')?.textContent?.includes('LOCAL EXECUTION') && document.querySelector('.policy-page')?.textContent?.includes('NO AUTO-SEND');"), "The public privacy route provides three readable operational ledger sections with local-only status readouts.");
