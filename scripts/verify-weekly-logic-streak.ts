...
assert(summary.completedDays === 3 && summary.totalCompletedFields === 4, "Two genuine fields on one day must count as one verified day but two completed fields.");
assert(completedFieldsForDate("2026-08-26", base).join(",") === "queens,zip", "The calendar must expose the actual completed genuine field types for a day.");
...
assert(current.currentStreak === 4 && current.longestStreak === 4 && current.completedDays === 4 && current.totalCompletedFields === 5, "A current genuine field must extend the local run once while retaining same-day aggregation.");
