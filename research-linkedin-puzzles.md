...
The local **Weekly Streak Calendar** is therefore an audit view of real browser-local successes, not a copied release schedule or engagement counter. It derives its marks only from existing completed Mini Sudoku, Zip, Tango, Queens, Patches, and Wend edition keys for device-local dates already reached. Its optional `All fields` or individual-game view is only an in-memory lens over that same verified record; it recalculates the visible markers and streak figures without saving a choice or sending any activity. Route opens, demo playback, a reflection/orientation, legacy fixed-board records, and future-dated local data never count; several completed genuine editions on one day remain one verified calendar day. No streak or activity data leaves the browser.
