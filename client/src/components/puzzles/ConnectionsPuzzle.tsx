...
export default function ConnectionsPuzzle() {
  ...
  const handleShare = () => {
    const shareUrl = typeof window !== "undefined" ? window.location.href : "https://toolboxgalaxy.com/games/connections";
    ...
    const text = `🔠 Connections #${edition.date}\n⏱️ Time: ${formatTime(seconds)}\n${emojiGrid}\nPlay in Toolbox Galaxy:\n${shareUrl}`;

    if (navigator.share) {
      navigator.share({
        title: `Toolbox Galaxy Connections`,
        text: text,
        url: shareUrl,
      }).catch(() => {
          ...
          <div className="mt-6 w-full p-5 rounded-2xl bg-gradient-to-b from-emerald-950/40 to-slate-900 border border-emerald-500/40 text-center space-y-3 shadow-xl animate-in zoom-in-95">
            ...
            <div className="flex justify-center gap-3 pt-2">
              <button
                type="button"
                onClick={handleShare}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#c7f36b] text-[#090d16] font-bold text-xs hover:bg-[#d6f685] transition-all active:scale-95"
              >
                <Share2 size={14} />
                <span>{copied ? "Copied Score to Clipboard!" : "Share Results"}</span>
              </button>
            </div>
          </div>
        )}
