...
export function completedFieldsForDate(dateId: string, completions: PuzzleCompletionMap, filter: WeeklyLogicFilter = "all"): GenuineDailyLogicSlug[] { return genuineDailyLogicSlugs.filter((game) => includesFilter(game, filter) && Object.keys(completions).some((key) => { const separator = key.lastIndexOf(":"); return separator >= 0 && key.slice(separator + 1) === dateId && key.slice(0, separator).startsWith(completionSlugFor(game)); })); }
