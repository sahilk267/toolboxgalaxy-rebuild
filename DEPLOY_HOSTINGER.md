...
After the `dist/public` upload, manually test all three routes: change group size/tip in Split Bill; verify a zero-rate and invalid-term path in Loan / EMI; and verify an overnight shift plus invalid break in Work Shift Duration. Check reset and copy feedback on each; inspect the mobile pages; and confirm the foundry telemetry reports 32 local modules. Continue to run `pnpm exec tsx scripts/verify-daily-tools.ts` and `pnpm exec tsx scripts/playtest-daily-tools.ts` with the regular release gate.
...
After uploading `dist/public`, open `/tools/json-csv-converter` and `/tools/csv-viewer-cleaner` directly on the live custom domain. Paste a flat JSON array with a comma and a newline in a value; change direction and verify a quoted CSV cell becomes JSON; then test malformed JSON and duplicate CSV headers. In the cleaner, paste padded rows plus a blank row, switch each clean setting, use Apply clean copy, and confirm its bounded table preview, reset, copy, and download feedback. Inspect both routes on mobile, confirm the Tool Foundry and quick-open label report 34 local modules, and run `pnpm exec tsx scripts/verify-structured-data-tools.ts`, `pnpm exec tsx scripts/verify-daily-tools.ts`, and `pnpm exec tsx scripts/playtest-daily-tools.ts` with the established static release gate.
