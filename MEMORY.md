...
- Every board is authored and deterministic. `scripts/verify-logic-puzzles.ts` verifies known solutions, representative invalid states, applicable uniqueness claims, Mini Sudoku’s three clue-density profiles, local daily-transform coordinate round trips, and every Zip bank edition before release.
- Patches is a 6×6 exact rectangle cover with one clue per rectangle; Zip is now a bank of seven distinct 6×6 non-repeating ordered orthogonal paths with unique solver results, scattered checkpoints, meaningful walls, and at least twenty turns; Wend is the confirmed orthogonal word-path exact cover, not a substituted word game.
- `daily.ts` derives a browser-local date ID, Calm/Standard/Dense profile, and reversible mirror orientation—no calendar API, remote daily feed, or account is involved. `zipBank.ts` turns that local date into one actual independently authored Zip edition and its board-specific completion key. Mini Sudoku profiles differ in solver-checked clue density; the other games still vary orientation and opening assistance while their rule contracts stay intact, so they are not yet substantive daily board banks.
